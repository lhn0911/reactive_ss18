import { getCarts, updateQuantityCart, deleteCartItem, deleteAllCart } from "@/apis/cart.api";
...
interface CartItemProps extends CartItemData {
  onUpdateQuantity: (action: 'increase' | 'decrease', cartItemId: number) => void;
  ...
  <View style={styles.itemContainer}>
    ...
    <View style={styles.itemDetails}>
      ...
      <View style={styles.quantityContainer}>
        <TouchableOpacity onPress={() => item.onUpdateQuantity("decrease", item.id)}>
          <Ionicons name="remove-circle-outline" size={28} color="#555" />
        </TouchableOpacity>
        <Text style={styles.quantityText}>{item.quantity}</Text>
        <TouchableOpacity onPress={() => item.onUpdateQuantity("increase", item.id)}>
...
const CartSummary = ({totalPrice}: {totalPrice: number}) => (
  <View style={styles.summaryContainer}>
    ...
    <View style={styles.summaryRow}>
      ...
      <TextInput keyboardType="numeric" style={styles.textInput} placeholder="0 VNĐ" />
    </View>
...
export default function CartScreen() {
  ...
  const { mutate: updateQuantity } = useMutation({
    ...
    onMutate: async (newData: { id: number; quantity: number }) => {
      ...
      queryClient.setQueryData(["carts"], (old: any) => {
        ...
        const updatedCartItems = old.data.cartItems.map((item: CartItemData) =>
          item.id === newData.id ? { ...item, quantity: newData.quantity } : item
        );
        const newTotalAmount = updatedCartItems.reduce((acc: any, item: CartItemData) => acc + (item.price * item.quantity), 0);
        return { ...old, data: { ...old.data, cartItems: updatedCartItems, totalAmount: newTotalAmount } };
      });
      return { previousCart };
    },
  ...
  if (isError) {
    return <Text style={{ textAlign: 'center', marginTop: 50 }}>Đã có lỗi xảy ra.</Text>;
  }

  const handleUpdateQuantity = (action: 'increase' | 'decrease', cartItemId: number) => {
    const item = cartItems.find((item: CartItemData) => item.id === cartItemId);
    if (!item) return;
  ...
  const handleDeleteItem = (cartItemId: number) => {
      Alert.alert(
          ...
          [
              ...
              { text: "Xóa", onPress: () => deleteItem(cartItemId), style: "destructive" }
          ]
      )
  };
